...
import { getLevel2LeaderboardEntries, type Level2LeaderboardEntry } from "@/lib/firebase/level2-leaderboard"
...
export default function ViewLevel2LeaderboardTable({ competitionId }: { competitionId: string }) {
  const [allEntries, setAllEntries] = useState<Level2LeaderboardEntry[]>([])
  ...
  const loadInitialData = async () => {
    ...
    try {
      const { entries: newEntries, lastDoc: newLastDoc } = await getLevel2LeaderboardEntries(competitionId, undefined, 50)
      setAllEntries(newEntries)
      setLastDoc(newLastDoc)
      setHasMore(newEntries.length === 50)
    } catch (error) {
      console.error("Error loading Level 2 leaderboard:", error)
    } finally {
  ...
  const loadMore = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const { entries: newEntries, lastDoc: newLastDoc } = await getLevel2LeaderboardEntries(competitionId, lastDoc, 25)
  ...
  const stats = useMemo(() => {
    ...
    const highestScore = allEntries.length > 0 ? Math.max(...allEntries.map((e) => e.finalScore)) : 0
    const averageScore =
      allEntries.length > 0 ? allEntries.reduce((sum, e) => sum + e.finalScore, 0) / allEntries.length : 0
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center space-y-3">
          ...
          <p className="text-gray-600">Loading Level 2 leaderboard...</p>
        </div>
      </div>
    )
  }
      ...
      <Card>
        <CardHeader className="pb-4">
          <CardTitle className="flex items-center gap-2">
            ...
            Level 2 Final Leaderboard ({processedEntries.length})
          </CardTitle>
        </CardHeader>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b bg-gray-50/50">
                    ...
                    <th className="text-right p-4 font-medium text-gray-700">Final Score</th>
                  </tr>
                </thead>
                <tbody>
                  {processedEntries.map((entry, index) => (
                    ...
                    >
                      ...
                      <td className="p-4 text-right">
                        <div className="font-semibold text-gray-900 text-lg">{entry.finalScore.toFixed(1)}</div>
                      </td>
                    </tr>
                  ))}
                </tbody>
