...
export const DailyChallengeCard = ({ challenge, onViewDetails, isSingle = false }: DailyChallengeCardProps) => {
  ...
  if (isSingle) {
    return (
      <Card className="bg-white border border-slate-200 shadow-sm hover:shadow-md transition-shadow duration-200 rounded-2xl overflow-hidden">
        <CardContent className="p-8">
          <div className="grid lg:grid-cols-[1fr,auto] gap-8 items-start">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 flex-wrap">
                  <Badge className="bg-[#0f172a] hover:bg-[#0d1220] text-white border-0 font-medium text-xs px-3 py-1.5 rounded-md">
                    <Flame className="w-3.5 h-3.5 mr-1.5" />
                    Daily Challenge
                  </Badge>
                  {isActive && (
                    <Badge className="bg-emerald-50 text-emerald-700 border border-emerald-200 font-medium text-xs px-3 py-1.5 rounded-md">
              ...
              <div className="flex items-center justify-between gap-3 p-4 bg-slate-50 rounded-xl border border-slate-200">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-slate-700 border-slate-300 hover:bg-slate-50 hover:text-slate-900 font-medium">Total Submissions: {challenge.totalSubmissions || 0}</p>
                ...
                <div>
                  <Dialog>
                    <DialogTrigger asChild>
                      <Button
                        variant="outline"
                        className="text-slate-700 border-slate-300 hover:bg-slate-50 hover:text-slate-900 font-medium"
                      >
                    ...
                    <DialogContent className="bg-white border-slate-200 max-w-3xl w-[95vw] max-h-[85vh] overflow-y-auto">
                      <DialogHeader className="space-y-3 pb-4 border-b border-slate-200">
                        <div className="flex items-start gap-3">
                          <div className="flex items-center justify-center w-11 h-11 bg-indigo-100 rounded-xl flex-shrink-0">
                            <Eye className="w-5 h-5 text-indigo-600" />
                          </div>
                          <div className="flex-1">
                            <DialogTitle className="text-xl font-bold text-slate-900">Challenge Details</DialogTitle>
                        ...
                        {(challenge.problemStatement || (challenge.problemAudioUrls && challenge.problemAudioUrls.length > 0)) && (
                          <div className="space-y-3">
                            <div className="flex items-center gap-2">
                              ...
                              <h4 className="text-base font-semibold text-slate-900">Problem Statement</h4>
                            </div>
                            {challenge.problemStatement && (
                        ...
                        {(challenge.guidelines || (challenge.guidelinesAudioUrls && challenge.guidelinesAudioUrls.length > 0)) && (
                          <div className="space-y-3">
                            <div className="flex items-center gap-2">
                              ...
                              <h4 className="text-base font-semibold text-slate-900">Guidelines</h4>
                            </div>
            ...
            <div className="lg:min-w-[320px] space-y-4 lg:mt-12">
              {/* Time Info Cards */}
              <div className="grid grid-cols-2 gap-3">
                {/* Time Left */}
                <div className="p-4 bg-gradient-to-br from-[#e0edfe]/80 to-[#e0edfe]/60 rounded-xl border" style={{ borderColor: 'rgba(224,237,254,0.6)' }}>
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-8 h-8 bg-[#123b8f] rounded-lg flex items-center justify-center">
                        <Clock className="w-4 h-4 text-white" />
                      </div>
                    </div>
                    <p className="text-xs font-medium text-[#123b8f] uppercase tracking-wide mb-1">Time Left</p>
                    <p className="text-xl font-bold text-[#123b8f]">{isActive ? timeLeft : "Ended"}</p>
                  </div>
                ...
                <div className="p-4 bg-gradient-to-br from-amber-50 to-amber-100/50 rounded-xl border border-amber-200">
                  ...
                  <p className="text-xs font-medium text-amber-700 uppercase tracking-wide mb-1">Deadline</p>
                  <p className="text-base font-bold text-amber-900">{endDateTime.date}</p>
                  <p className="text-xs font-medium text-amber-700 mt-0.5">{endDateTime.time}</p>
                </div>
              </div>
  ...
  return (
    <Card className="bg-white border border-slate-200 rounded-xl overflow-hidden hover:shadow-md hover:border-slate-300 transition-all duration-200 h-full flex flex-col">
      <CardContent className="p-5 flex flex-col h-full">
        {/* Header Section */}
        <div className="space-y-3 mb-4">
          {/* Badges */}
          <div className="flex items-center gap-2 flex-wrap">
            <Badge className="bg-[#0f172a] hover:bg-[#0d1220] text-white border-0 font-medium text-xs px-2.5 py-1 rounded-md">
              <Flame className="w-3 h-3 mr-1" />
              Daily
            </Badge>
            {isActive && (
              <Badge className="bg-emerald-50 text-emerald-700 border border-emerald-200 font-medium text-xs px-2.5 py-1 rounded-md">
        ...
        {/* Info Grid */}
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="flex items-start gap-2 p-3 bg-slate-50 rounded-lg border border-slate-200">
              <div className="w-8 h-8 bg-[#123b8f] rounded-lg flex items-center justify-center flex-shrink-0">
                <Clock className="w-4 h-4 text-white" />
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-xs font-medium text-slate-600 mb-0.5">Time Left</p>
                <p className="text-sm font-bold text-[#123b8f]">{isActive ? timeLeft : "Ended"}</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-2 p-3 bg-slate-50 rounded-lg border border-slate-200">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-xs font-medium text-slate-600 mb-0.5">Deadline</p>
                <p className="text-sm font-bold text-amber-900 leading-tight">{endDateTime.date}</p>
              </div>
            </div>
          ...
          <div className="flex items-start gap-2 p-3 bg-slate-50 rounded-lg border border-slate-200">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-xs font-medium text-slate-600 mb-0.5">Submissions</p>
              <p className="text-sm font-bold text-slate-900">{challenge.totalSubmissions || 0}</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-2 p-3 bg-slate-50 rounded-lg border border-slate-200">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-xs font-medium text-slate-600 mb-0.5">Type</p>
              <p className="text-sm font-bold text-slate-900 capitalize">{challenge.type}</p>
            </div>
          </div>
        </div>
        ...
        <Button
          onClick={() => router.push(`/participant/daily-challenge/${challenge.id}`)}
          disabled={!isActive}
          className={`w-full h-10 text-sm font-semibold rounded-lg transition-all duration-200 mt-auto ${
            isActive
              ? hasSubmission
