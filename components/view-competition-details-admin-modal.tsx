...
interface ViewCompetitionDetailsAdminModalProps {
  isOpen: boolean
  onClose: () => void
  competition: CompetitionAdmin | null
}

export const ViewCompetitionDetailsAdminModal = ({ isOpen, onClose, competition }: ViewCompetitionDetailsAdminModalProps) => {
          ...
          {competition.systemPrompt && (
            <div className="bg-indigo-50 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                <Label className="text-base font-semibold text-indigo-900">System Prompt</Label>
              </div>
              <div className="text-sm text-gray-900 font-medium break-words whitespace-pre-wrap bg-white p-3 rounded border max-h-44 overflow-y-auto">
                {competition.systemPrompt}
              </div>
            </div>
          )}
