...
import { submitPrompt, checkExistingSubmission, fetchBatchDetails } from "@/lib/api"
...
export default function Level2ChallengePage() {
  ...
  const [batchEndDeadline, setBatchEndDeadline] = useState<Date | null>(null)
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        await Promise.all([
          fetchChallengeData(id, challengeId, profile.uid),
    ...
    const checkEnd = async () => {
      if (user) {
        const ended = await checkBatchEnded(user.uid)
        setIsBatchEnded(ended)
      }
    }
  ...
  const checkBatchEnded = async (participantId: string) => {
    try {
      const batchData = await fetchBatchDetails(competitionId, participantId)
      ...
      console.error("Error checking batch end date:", error)
      return false
    }
  }

  const fetchChallengeData = async (competitionId: string, challengeId: string, participantId: string) => {
    try {
      setLoadingChallenge(true)

      // Fetch batch details to get batch-specific deadlines
      const batchData = await fetchBatchDetails(competitionId, participantId)
      ...
      const isActive = competitionData.isActive || false
      setIsCompetitionActive(isActive)

      // Use batch end time instead of competition end time
      const batchEnd = endTime ? new Date(endTime) : null
      const isBatchLocked = batchEnd && new Date() > batchEnd
      
      setBatchEndDeadline(batchEnd)
      ...
      const challengeData: Challenge = {
        ...
        isCompetitionLocked: isBatchLocked || false,
        endDeadline: batchEnd as any, // Use batch end time
          ...
          <Card className="bg-white shadow-2xl border border-gray-100 rounded-xl max-w-md">
            <CardContent className="text-center py-12 px-8">
              ...
              <Button
                ...
                onClick={() => router.push(`/participant/${compid}/level2`)}
              >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                  ...
                  {batchEndDeadline && (
                    <div className="flex items-center gap-3 text-sm mt-2 sm:mt-0 sm:ml-auto bg-white rounded-xl px-4 py-2 border border-gray-200 shadow-sm">
                      <CountdownDisplay 
                        targetDate={batchEndDeadline} 
                        onExpire={handleBatchExpiry}
                      />
                    </div>
                  )}
                </div>
              ...
              <CardContent className="p-6 space-y-6">
                {loadingPrompt ? (
                  ...
                  <>
                    ...
                    {(isBatchEnded || !isCompetitionActive) && (
                      <div className="flex items-center gap-2 text-sm text-red-600 bg-red-50 px-3 py-2 rounded-lg border border-red-200 mb-2 mt-4">
                        ...
                        <span>
                          {isBatchEnded 
                            ? "Submission time for your batch is over. You can no longer submit or update responses for this challenge."
                    ...
                    <div className="flex items-center justify-between pt-2">
                      ...
                      <Button
                        ...
                        disabled={!prompt.trim() || isBatchEnded || loading || !isCompetitionActive}
                        title={
                          isBatchEnded 
                            ? "Batch submission time is over" 
                            : !isCompetitionActive 
                        ...
                        <div className="bg-white rounded-xl shadow-2xl p-8 w-[400px] text-center border border-gray-100">
                          ...
                          <div className="flex justify-center gap-3">
                            <Button
                              ...
                              onClick={async () => {
                                const hasEnded = await checkBatchEnded(user.uid)
                                if (hasEnded) {
                                  setIsConfirmModalOpen(false)
                                  setSubmissionStatus("error")
                                  setSubmissionError("Batch submission time is over. You can no longer submit responses for this challenge.")
                                  return
                                }
                                ...
                                try {
                                  ...
                                  if (!result.success) {
                                    ...
                                    if (errorMessage.includes("Competition has ended") || errorMessage.includes("Batch has ended")) {
                                      errorMessage = "Batch submission time is over. You can no longer submit responses for this challenge."
                                    } else if (errorMessage.includes("Competition has not started yet")) {
                                      errorMessage = "The batch has not started yet. Please wait until it begins."
                                  ...
                                  if (error.message && (error.message.includes("Competition has ended") || error.message.includes("Batch has ended"))) {
                                    errorMessage = "Batch submission time is over. You can no longer submit responses for this challenge."
                      ...
                      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                        <div className="bg-white rounded-xl shadow-2xl p-8 w-[400px] text-center border border-gray-100">
                          ...
                          <Button
                            className="bg-gray-900 hover:bg-gray-800 text-white font-semibold px-6 py-2 rounded-xl transition-colors duration-200"
                            onClick={() => {
                              router.push(`/participant/${routeParams.competitionId}/level2`)
                            }}
                          >
                            Done
                          </Button>
