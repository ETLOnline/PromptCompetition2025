...
import { doc, setDoc, getDocs, Timestamp, collection, getDoc, updateDoc } from "firebase/firestore"
        ...
        {existingUrls.map((url, i) => (
          <div key={`existing-audio-${i}`} className="bg-gray-50 border border-gray-200 rounded-md p-3">
            <div className="flex items-center justify-between mb-2">
              <div className="text-sm font-medium text-gray-700">Existing Audio {i + 1}</div>
...
export default function NewDailyChallengeePage() {
  ...
  const fetchChallenge = async () => {
    try {
      ...
      setPageLoading(true)
      const docRef = doc(db, "dailychallenge", challengeId)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!hasProblemText && !hasProblemAudio) {
      console.warn('❌ Missing problem statement')
      toast({
    ...
    if (!hasGuidelinesText && !hasGuidelinesAudio) {
      console.warn('❌ Missing guidelines')
      toast({
    ...
    try {
      ...
      if (guidelinesAudioFiles.length > 0) {
        console.log('📤 Uploading guidelines audio files...')
        for (let i = 0; i < guidelinesAudioFiles.length; i++) {
          const file = guidelinesAudioFiles[i]
          console.log(`📤 Uploading guidelines audio ${i + 1}/${guidelinesAudioFiles.length}: ${file.name}`)
          try {
            const url = await uploadFile(file, 'voice')
            console.log(`✅ Guidelines audio uploaded: ${url}`)
            finalGuidelinesAudioUrls.push(url)
            setUploadProgress((p) => ({ ...p, current: p.current + 1 }))
          } catch (err: any) {
            console.error(`❌ Guidelines audio upload failed:`, err)
      ...
      if (visualClueFiles.length > 0) {
        console.log('📤 Uploading visual clue images...')
        for (let i = 0; i < visualClueFiles.length; i++) {
          const file = visualClueFiles[i]
          console.log(`📤 Uploading visual clue ${i + 1}/${visualClueFiles.length}: ${file.name}`)
          try {
            const url = await uploadFile(file, 'image')
            console.log(`✅ Visual clue uploaded: ${url}`)
            finalVisualClueUrls.push(url)
            setUploadProgress((p) => ({ ...p, current: p.current + 1 }))
          } catch (err: any) {
            console.error(`❌ Visual clue upload failed:`, err)
      ...
      await updateDoc(doc(db, "dailychallenge", challengeId), updateData)
        ...
        <div className="max-w-4xl mx-auto px-6">
          <div className="flex items-center justify-between py-6">
            <div className="flex items-center gap-4">
              <div>
                <h1 className="text-2xl font-semibold text-gray-900">Edit Daily Challenge</h1>
                <p className="text-sm text-gray-500">Update challenge details and requirements</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {(formData.visualClueUrls || []).map((url, i) => (
                    <div key={`existing-visual-${i}`} className="relative group">
                ...
                {visualClueFiles.length === 0 && (formData.visualClueUrls || []).length === 0 && (
